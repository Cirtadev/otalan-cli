...
describe('CLI help', () => {
  test('help reflects the release workflow', async () => {
    ...
    expect(result.stdout).toContain('List release channels for the authenticated project.')
    ...
    expect(result.stdout).toContain('Official app support: Capacitor 7/8 and Expo SDK 54/55/56.')
    expect(result.stdout).toContain('App-scoped release commands require the configured app to be active, not archived.')
